...
namespace transport_catalogue {
    ...
    void TransportCatalogue::AddDistance(const domain::Stop* ptr1, const domain::Stop* ptr2, double dist) {
    ...
    int TransportCatalogue::GetDistance(const domain::Stop* ptr1, const domain::Stop* ptr2) const {
    ...
    void TransportCatalogue::AddBus(const std::string& name, const std::vector<std::string>& stops, bool is_roundtrip) {
        ...
        for (const auto& stop : stops) {
            domain::Stop* stop1 = const_cast<domain::Stop*>(FindStop(stop));
            if (stop1) {
    ...
    std::optional<domain::BusInfo> TransportCatalogue::GetBusInfo(std::string_view bus_name) const {
        domain::BusInfo info;
        const domain::Bus* it_bus = FindBus(bus_name);
        //если не найдено
        if (!it_bus) {
            return std::nullopt;
        }
